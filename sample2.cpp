...
const float BOXSIZE = 5.f;

// multiplication factors for input interaction:
//  (these are known from previous experience)

const float ANGFACT = 2.f;
...
GLuint BoxList, WireHorseList;		 // object display list
...
int selectView;		//inside or outside
...
// void rockHorse(int state)
// {
//     glutPostRedisplay();
//     glutTimerFunc(30, rockHorse, 0);
// }
...
void Display()
{
	...
	glLoadIdentity();
	if (NowProjection == ORTHO && selectView == 0)
	...
	if(selectView == 0){
		...
		gluLookAt(0.f, 0.f, 0.f, 1.f, 0.f, 1.f, 0.f, 1.f, 0.f);
	}
	...
	glPushMatrix();
	//below lines for rocking of horse
		//glTranslatef(2, 2, 2);
		//glTranslatef(2, -2, 2);
	// glutPostRedisplay();

	// glutTimerFunc(30, rockHorse, 1);


	//currentTime = 
	rockingAngle = 35 * sin(2 * glutGet(GLUT_ELAPSED_TIME) / 300);
	verticalOffset = sin(2 * glutGet(GLUT_ELAPSED_TIME) / 300);

	glTranslatef(0,verticalOffset, 0);
	glRotatef( -360.f*Time,   0., 1., 0. );
	glTranslatef(2, 0, 0);
	glRotatef(rockingAngle, 1, 0, 0);
	//glTranslatef(0, verticalOffset, 0);
...
void InitLists()
{
	...
	glColor3f(1.0, 0, 0);
    DrawCircle(0.5, 0.5, 2, 100);

	glColor3f(0, 1.0, 1.0);
	glDrawArrays(GL_LINE_LOOP, 0, HORSEnpoints);
